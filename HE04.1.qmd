...
There are several ways to plot ridgeline plot with R. This section discusses how to plot ridgeline plot by using [ggridges](https://wilkelab.org/ggridges/index.html) package.
...
At times, the area beneath a ridgeline may be preferred not to be filled with a single solid color but instead with colors that vary along the x-axis. This effect can be produced by using either [geom_ridgeline_gradient()](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html) or [geom_density_ridges_gradient()](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html). Both geoms are used in the same way as geom_ridgeline() and geom_density_ridges(), except that variation in fill colors is permitted. However, alpha transparency in the fill is not supported. Due to technical limitations, either varying fill colors or transparency can be applied—but not both simultaneously.
...
The Raincloud Plot is regarded as a data visualisation technique that produces a half-density representation of a distribution plot. Its name is derived from the density plot’s resemblance to a “raincloud.” The traditional boxplot is enhanced by the raincloud (half-density) plot, as multiple modalities—indicators of potential subgroup existence—are highlighted. While the boxplot does not display where densities are concentrated, such clustering is revealed by the raincloud plot. In this section, the process of creating a raincloud plot to visualise the distribution of English scores by race will be demonstrated. The plot will be generated using functions provided by the **ggdist** and ggplot2 packages.
...
Next, the second geometry layer will be added using [`geom_boxplot()`](https://r4va.netlify.app/chap09) of ggplot2. This produces a narrow boxplot. The width is reduced and the opacity is adjusted.
...
Next, the third geometry layer is added using [`stat_dots()`](https://mjskay.github.io/ggdist/reference/stat_dots.html) of ggdist package. This produces a half-dotplot, which is similar to a histogram that indicates the number of samples (number of dots) in each bin. The side = “left” is selected to put it on the left-hand side.
